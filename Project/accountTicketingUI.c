...
void applicationStartup(struct Account accounts[], int arrSize)
{
    ...
    while (1)
    {
        ...
        {
            // index is returned only when the maching account type is agent
...
int menuLogin(const struct Account accounts[], int arrSize)
{
    ...
    do
    {
        ...
        else
        {
            ...
            // either no match is found or the matching account is not an Agent
            if (accountType != 'A')
            {
...
void menuAgent(struct Account accounts[], int arrSize, const struct Account* account)
{
    ...
    struct Account accountInfo = *account; // logged-in user's account
    int option = -1;
    ...
    while (1)
    {
        ...
        if (option == 1)
        {
            ...
            for (i = 0; i < arrSize; i++)
            {
                if (accounts[i].accountNum == 0)
                {
                    ...
                    getAccount(&account); // fill up account with user inputs using its address
                    
                    accounts[i] = account;
                    accountAdded = 1; // indication of new record is added to array
                    break; // once adding a new account, get out of the loop
                }
            }
        ...
        else if (option == 2)
        {
            ...
            if (accountFound)
            {
                ...
                // Added new vvvvvv
                //accountInfo = accounts[index]; // logged-in user's account
        ...
        else if (option == 3)
        {
            ...
            if (accountFound)
            {
                ...
                {
                    // Call function to display the account detail header
                    displayAccountDetailHeader();
                    // Call function to display an account detail record
...
int findAccountIndexByAcctNum(int accountNum, 
                              const struct Account accounts[], int maxNum, int isPrompt)
{
